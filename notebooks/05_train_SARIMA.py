...
output_path = "outputs/sarima_results.csv"
...
print("\nData prepared for SARIMA")
print(f"Train values: {len(train_sales)}")
print(f"Test values: {len(test_sales)}")

# Train SARIMA model
# The seasonal period "7" is used because the data is daily and weekly patterns are expected.
...
print("\nError Metrics - SARIMA(1,1,1)(1,1,1,7)")
print("=" * 45)
print(f"MAE: {mae:.2f}")
...
result_sarima = pd.DataFrame(
    {
        ...
        "sarima_predictions": predictions,
    }
)
result_sarima.to_csv(output_path, index=False)

print(f"\nSARIMA results saved at: {output_path}")

# Show first predictions
print(f"\nFirst 10 predictions vs real values:")
print(result_sarima.head(10))
